...
from pyaedt.modules.FieldCalculator import (
    Acos,
    Asin,
    AtPhase,
    Atan,
...
def test_complex_vector():
    check_call_sequence(Vector(complex(4,5), complex(6,7), complex(8,9)), [["EnterComplexVector", ["4.0 + 5.0j", "6.0 + 7.0j", "8.0 + 9.0j"]]])
...
def test_complicated_expression():
    check_call_sequence(
        ...
        Integrate(quantity.Energy, surface.region) * scalar.HV_LV_gap_length) * 2 / (scalar.HV_current * scalar.HV_current),
        [
            ["EnterQty", "Energy"],
            ["EnterSurface", "HV"],
            ["CalcOp", "Integrate"],
