...
class TestClass:
    ...
    def test_parse_dim_arg(self):
        ...
        assert go.parse_dim_arg('2mm', 'mm') == 2.
    ...
    def test_get_mid_point(self):
        ...
        assert is_vector_equal(m, [5., 5., 5.])
    ...
    def test_v_cross(self):
        ...
        v2 = [0., 1., 0.]
        v3 = [2., 0., 0.]
        assert is_vector_equal(go.v_cross(v1, v2), [0., 0., 1.])
        assert is_vector_equal(go.v_cross(v2, v1), [0., 0., -1.])
        assert is_vector_equal(go.v_cross(v1, v3), [0., 0., 0.])
    ...
    def test_v_dot(self):
        ...
        v3 = [2., 0., 0.]
        v4 = [-1., -1., 0.]
        assert go.v_dot(v1, v2) == 0.
        assert go.v_dot(v1, v3) == 2.
        assert go.v_dot(v1, v4) == -1.
    ...
    def test_v_sub(self):
        v1 = [1., 0., 1.]
        v2 = [0., 1., 1.]
        assert is_vector_equal(go.v_sub(v1, v2), [1., -1., 0.])
        assert is_vector_equal(go.v_sub(v2, v1), [-1., 1., 0.])

    def test_v_sum(self):
        v1 = [1., 0., 1.]
        v2 = [0., 1., 1.]
        assert is_vector_equal(go.v_sum(v1, v2), [1., 1., 2.])
        assert is_vector_equal(go.v_sum(v2, v1), [1., 1., 2.])
    ...
    def test_v_points(self):
        p1 = [1., 0., 1.]
        p2 = [0., 1., 1.]
        assert is_vector_equal(go.v_points(p1, p2), [-1., 1., 0.])
    ...
    def test_rad2deg(self):
        assert abs(go.rad2deg(math.pi) - 180.) < tol
    ...
    def test_q_prod(self):
        ...
        q = go.q_prod(q1, q2)
        assert is_vector_equal(
            q, [0.6916264024663118, -0.1733462058496682, -0.7002829056219277, 0.03475434394060616])
        assert abs(q[0]**2 + q[1]**2 + q[2]**2 + q[3]**2 - 1.) < tol
