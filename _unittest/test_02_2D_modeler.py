...
class TestClass:
    ...
    def test_14_split(self):
        ...
        assert not self.aedtapp.modeler.split(rect1)
        split = self.aedtapp.modeler.split(rect1, plane=self.aedtapp.PLANE.ZX)
        assert isinstance(split, list)
        assert isinstance(split[0], str)
        obj_split = [obj for obj in self.aedtapp.modeler.object_list if obj.name == split[1]][0]
        assert not self.aedtapp.modeler.split(obj_split, tool=poly1.edges[0])
