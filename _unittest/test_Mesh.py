...
class TestMesh:
    ...
    def test_curvature_extraction(self):
        ...
        curv_extr =self.aedtapp.mesh.assign_curvature_extraction("inner")
        assert curv_extr

    def test_maxwell_mesh(self):
        m3d = Maxwell3d()
        id1 = m3d.modeler.primitives.create_box([0,0,0], [10,10,10], name="Box_Mesh")
        rot = m3d.mesh.assign_rotational_layer("Box_Mesh", meshop_name="Rotational")
        edge = m3d.mesh.assign_edge_cut("Box_Mesh", meshop_name="Edge")
        density = m3d.mesh.assign_density_control("Box_Mesh", maxelementlength=10000, meshop_name="Density")
        assert rot
        assert edge
        assert density
