...
class TestClass(BasisTest, object):
    def setup_class(self):
        ...
        self.aedtapp = BasisTest.add_app(self, original_project_name, application=Circuit, subfolder=examples_subfolder)
        self.circuitprj = BasisTest.add_app(
            self, "differential_pairs", application=Circuit, subfolder=examples_subfolder
        )
        netlist_file1 = os.path.join(local_path, "example_models", examples_subfolder, netlist1)
        netlist_file2 = os.path.join(local_path, "example_models", examples_subfolder, netlist2)
        touchstone_file = os.path.join(local_path, "example_models", examples_subfolder, touchstone)
        touchstone_file2 = os.path.join(local_path, "example_models", examples_subfolder, touchstone2)
    ...
    def test_20_create_AMI_plots(self):
        ami_design = BasisTest.add_app(
            self, ami_project, design_name="Models Init Only", application=Circuit, subfolder=examples_subfolder
        )
    ...
    def test_25_import_model(self):
        ...
        touch = os.path.join(local_path, "example_models", examples_subfolder, "SSN_ssn.s6p")
    ...
    def test_28_load_and_save_diff_pair_file(self):
        diff_def_file = os.path.join(
            local_path, "example_models", examples_subfolder, "differential_pairs_definition.txt"
        )
