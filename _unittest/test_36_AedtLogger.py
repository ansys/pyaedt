...
class TestClass:
    ...
    def test_02_app_filter(self):
        content = None
        #t = tempfile.NamedTemporaryFile(mode="w+t", suffix=".txt", delete=False)
        #fd, path = tempfile.mkstemp(suffix=".txt", text=True)
        path = r"C:\Users\mrey\AppData\Local\Temp\toto.txt"
        with open(path, 'w') as f:
            ...
            logger.glb.info("Yes")
            logger.glb.debug("Yes")
            logger.glb.warning("Yes")
        ...
        #     content = f.readlines()
        #     assert "Info for Project" in content
