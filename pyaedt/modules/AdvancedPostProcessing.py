...
class PostProcessor(Post):
    ...
    def export_model_obj(self):
        """Export the model."""
        assert self._p_app._aedt_version >= "2021.2", self._messenger.add_error_message(
            "Object is supported from AEDT 2021 R2."
        )
        project_path = self._p_app.project_path
        obj_list = self._p_app.modeler.primitives.object_names
        obj_list = [
            i
            for i in obj_list
            if not self._p_app.modeler.primitives.objects[self._p_app.modeler.primitives.get_obj_id(i)].is3d
            or (
                self._p_app.modeler.primitives.objects[
                    self._p_app.modeler.primitives.get_obj_id(i)
                ].material_name.lower()
                != "vacuum"
                and self._p_app.modeler.primitives.objects[
                    self._p_app.modeler.primitives.get_obj_id(i)
                ].material_name.lower()
                != "air"
            )
        ]
        self._p_app.modeler.oeditor.ExportModelMeshToFile(os.path.join(project_path, "Model.obj"), obj_list)
    ...
    def export_mesh_obj(self, setup_name=None, intrinsic_dict={}):
        ...
        if not setup_name:
            setup_name = self._p_app.nominal_adaptive
        face_lists = []
        obj_list = self._p_app.modeler.primitives.object_names
        for el in obj_list:
            obj_id = self._p_app.modeler.primitives.get_obj_id(el)
            if not self._p_app.modeler.primitives.objects[obj_id].is3d or (
                    self._p_app.modeler.primitives.objects[obj_id].material_name != "vacuum"
                    and self._p_app.modeler.primitives.objects[obj_id].material_name != "air"
            ):
                face_lists += self._p_app.modeler.primitives.get_object_faces(obj_id)
