...
class Cable:
    def __init__(self, app, json_file_name=None):
        self._app = app
        self._odesign = app.odesign
        self._omodule = self._odesign.GetModule("CableSetup")

        file_import = self._cable_properties_parser(self._omodule)
    ...
    def create_cable(self):
        if self.cable_type == "bundle":
            ...
            if self.is_lay_length_specified.lower() == "true":
                ...
                msg = "is_lay_length_specified value not valid. Value must be either True or False."
                ...
                [
                    ...
                    [
                        ...
                        [
                            ...
                            ["NAME:WireType", "Value:=", self.wire_type],
    ...
    def remove_cable(self, cables_list_to_remove):
        for cable_to_remove in cables_list_to_remove:
            if cable_to_remove not in itertools.chain(
                self.existing_bundle_cables_names,
                self.existing_straight_wire_cables_names,
                self.existing_twisted_pair_cables_names,
            ):
                msg = "cable doesn't exist in the project."
                raise ValueError(msg)
            else:
                self._omodule.RemoveCable(cable_to_remove)

    def add_cable_to_bundle(self, cable_to_add, bundle_cable_name, number_of_cable_to_add):
        if cable_to_add in self.existing_straight_wire_cables_names:
            self._omodule.AddCableToBundle(
                bundle_cable_name,
                cable_to_add,
                number_of_cable_to_add,
    ...
    def _init_from_json(self, json_file_name):
        ...
        if self.cable_type == "bundle":
            ...
            if bool_jacket_type_list.count(True) > 1:
                ...
                if bool_jacket_type_list[0]:
                    ...
                    if [
                        ...
                        self.insulation_thickness = cable_bundle_properties["BundleParams"]["InsulationJacketParams"][
                            "InsThickness"
                        ]
                        if self._app.materials.material_keys.get(
                            cable_bundle_properties["BundleParams"]["InsulationJacketParams"]["JacketMaterial"].lower()
                        ):
                            self.jacket_material = cable_bundle_properties["BundleParams"]["InsulationJacketParams"][
                                "JacketMaterial"
                            ]
                        else:
                ...
                # Bundle Jacket type : Braided Shield
                    ...
                    if [
                        ...
                        if self._app.materials.material_keys.get(
                            cable_bundle_properties["BundleParams"]["BraidShieldJacketParams"]["JacketMaterial"].lower()
                        ):
                            self.jacket_material = cable_bundle_properties["BundleParams"]["BraidShieldJacketParams"][
                                "JacketMaterial"
                            ]
                        else:
                            msg = "material provided doesn't exist."
                        ...
                        self.num_wires_carriers = cable_bundle_properties["BundleParams"]["BraidShieldJacketParams"][
                            "NumWiresInCarrier"
                        ]
                ...
                # Bundle Jacket type : no jacket
                    ...
                    if [
                        ...
                        if cable_bundle_properties["BundleParams"]["VirtualJacketParams"][x] in ("", [], None, 0, False)
                    ]:
            ...
            if not self.existing_straight_wire_cables_names:
                msg = (
                    "no straight wire cables existing in the project. At least two are needed to create a twisted pair."
                )
                raise ValueError(msg)
            else:
                if (
                    ...
                    msg = "cable provided doesn't exist in the current project. Please provide an existing cable id."
                    raise ValueError(msg)
                    # Set default values if one or more keys are empty
                if [
                    x
    ...
    # FIX: WHERE TO STORE JSON?
    def _cable_properties_parser(self, omodule):
        current_location = os.path.dirname(os.path.abspath(__file__))
        file_path_export = os.path.join(current_location, "export_cable_library_test.txt")
        omodule.ExportCableLibrary(file_path_export)
        file_path_export_as_json = os.path.join(current_location, "export_cable_library_as_json_test.json")
