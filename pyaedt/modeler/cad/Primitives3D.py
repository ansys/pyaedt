...
class Primitives3D(GeometryModeler):
    ...
    def _create_cylinder(self, df, unit):
        ...
        for i in range(len(df)):
            ...
            if inner_radius == 0:
                try:
                    outer_cylinder = self.create_cylinder(cs_axis, position, radius, height, name=name)
                except Exception:  # pragma: no cover
                ...
                try:
                    outer_cylinder = self.create_cylinder(cs_axis, position, radius, height, name=name)
                    inner_cylinder = self.create_cylinder(cs_axis, position, inner_radius, height, name=name + "_inner")
