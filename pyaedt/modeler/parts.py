...
class Part(object):
    ...
    def __init__(self, part_folder, part_dict, parent=None, name=None):
        ...
        if parent:  # Inherit _motion directly from parent.
            self._motion = self._p_multiparts.motion
    ...
    def cs_name(self):
        ...
        if self._motion or not self.zero_offset('offset') or not self.zero_offset('rotation_cs'):
            ...
            return self._p_multiparts.cs_name
    ...
    def local_origin(self):
        ...
        if self['offset']:
            if self.zero_offset('offset') or self['offset'] == 'Global':
                ...
                if self._p_multiparts._local_units:
                    units = self._p_multiparts._local_units
                else:
                    units = self._p_multiparts.modeler_units
    ...
    def name(self):
        ...
        str
            Name of the part.
        """
        return self._p_multiparts.name + '_' + self._name
    ...
    def set_relative_cs(self, app):
        ...
        if self.cs_name not in app.modeler.oeditor.GetCoordinateSystems() and self.cs_name != "Global":
            ...
            app.modeler.create_coordinate_system(origin=self.local_origin,
                                                 ...
                                                 reference_cs=self._p_multiparts.cs_name,
                                                 mode="axis",
                                                 name=self.cs_name)
        return True
    ...
    def insert(self, app):
        ...
        if self._do_offset:
            ...
            aedt_objects.append(
                app.modeler.primitives.insert_3d_component(self.file_name, targetCS=self._p_multiparts.cs_name))
        if self._do_rotate:
            self.do_rotate(app, aedt_objects[0])

        # Duplication occurs in parent coordinate system.
        app.modeler.set_working_coordinate_system(self._p_multiparts.cs_name)
...
class Antenna(Part, object):
    ...
    def _insert(self, app, target_cs=None, units=None):
        if not target_cs:
            target_cs = self._p_multiparts.cs_name
        if not units:
            if self._p_multiparts._local_units:
                units = self._p_multiparts._local_units
            else:
    ...
    def insert(self, app, units=None):
        ...
        if self._do_offset:
            ...
            antenna_object = self._insert(app, target_cs=self._p_multiparts.cs_name, units=units)
