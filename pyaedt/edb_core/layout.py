...
class EdbLayout(object):
    ...
    @pyaedt_function_handler
    def create_soldermask_opening(self, reference_designator=None, expansion=1e-4):
        """Create openings on soldermask layers

        Parameters
        ----------
        reference_designator: str, list
            Components to create soldermask openings for. If it is None, create
            openings for all components. Default is `None`.
        expansion: float
            Incremental size of opening to component bounding box.
        ...
        if not reference_designator:
            reference_designator = list(self._pedb.core_components.components.keys())
        elif not isinstance(reference_designator, list):
            reference_designator = [reference_designator]
        else:
            pass

        soldermask = dict()
        for refdes in reference_designator:
            comp = self._pedb.core_components.components[refdes]

            bounding_box = comp.bounding_box
            opening = [bounding_box[0] - expansion]
            opening.append(bounding_box[1] - expansion)
            opening.append(bounding_box[2] + expansion)
            opening.append(bounding_box[3] + expansion)

            comp_layer = comp.placement_layer
            ...
            if soldermask_layer not in soldermask:
                soldermask[soldermask_layer] = list()

            void = self.create_rectangle(
                soldermask_layer,
                "{}_opening".format(refdes),
                lower_left_point=opening[:2],
                upper_right_point=opening[2:],
            )

            soldermask[soldermask_layer].append(void)

        for sm, vds in soldermask.items():
            all_nets = list(self._pedb.core_nets.nets.values())
            poly = self._pedb._create_conformal(all_nets, 0, 1e-12, False, 0)
            temp = self.create_polygon(poly, sm, [], "")
            self.add_void(temp, vds)
        return True
