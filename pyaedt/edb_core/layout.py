...
class EdbLayout(object):
                ...
                if prev_point != point:
                    ...
                    if check_inside:
                        ...
                        new_points = self._edb.Geometry.PointData(
                            self._edb_value(
                                point.X.ToString()
                                + "{}*{}".format(xcoeff, offset_name),
                                var_server,
                            ),
                            self._edb_value(
                                point.Y.ToString()
                                + "{}*{}".format(ycoeff, offset_name),
                                var_server,
                            ),
                        )
    ...
    def create_path(
        ...
        pointlists = [
            self._edb.Geometry.PointData(self._edb_value(i[0]), self._edb_value(i[1]))
    ...
    def _createPolygonDataFromRectangle(self, shape):
        ...
        pointA = self._edb.Geometry.PointData(
            self._edb_value(shape.pointA[0]), self._edb_value(shape.pointA[1])
        )
        pointB = self._edb.Geometry.PointData(
            self._edb_value(shape.pointB[0]), self._edb_value(shape.pointB[1])
        )
