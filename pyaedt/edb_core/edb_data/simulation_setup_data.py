...
class SiwaveSimulationSetup(object):
    """Manages EDB methods for hfss simulation setup."""
    ...
    def __init__(self, edb, name=None, edb_siwave_sim_setup=None):
        ...
        if edb_siwave_sim_setup:
            self._edb_sim_setup = edb_siwave_sim_setup
            self._edb_sim_setup_info = edb_siwave_sim_setup.GetSimSetupInfo()
        else:
            self._edb_sim_setup_info = self._edb.simsetupdata.SimSetupInfo[
    ...
    def _update_setup(self):
        self._edb_sim_setup_info.SweepDataList.Clear()
        for sweep in self.frequency_sweeps.items():
            self._edb_sim_setup_info.SweepDataList.Add(sweep)
        self._edb_sim_setup = self._edb.edb.Utility.SIWaveSimulationSetup(self._edb_sim_setup_info)
        if self._name in self._edb.simulation_setups.setups:
            self._edb._active_layout.GetCell().DeleteSimulationSetup(self._name)
        self._edb._active_layout.GetCell().AddSimulationSetup(self._edb_sim_setup)
    ...
    def advanced_settings(self):
        return SiwaveAdvancedSettings(self)
