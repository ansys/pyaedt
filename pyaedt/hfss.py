...
class Hfss(FieldAnalysis3D, object):
    ...
    def export_results(self, analyze=False, export_folder=None):
        ...
        for rName in reportnames:
            ...
            self.post.ofieldsreporter.ExportToFile(str(rName), szExportPath)
        ...
        for s in setups:
            ...
            if len(sweeps) == 0:
                ...
                pass
            for sweep in sweeps:
                variation_array = self.osolution.ListVariations("{0}:{1}".format(s, sweep))
                if len(variation_array) == 1:
                    szExportPath = os.path.join(export_folder, "{}.prof".format(self.project_name))
                    self.odesign.ExportProfile(s, variation_array[0], szExportPath)
                    self.logger.info("Export Profile: %s", szExportPath)
                    szExportPath = os.path.join(export_folder, "{}.conv".format(self.project_name))
                    self.odesign.ExportConvergence(s, variation_array[0], szExportPath)
                    self.logger.info("Export Convergence: %s", szExportPath)
                    try:
                        szExportPath = os.path.join(export_folder, "{0}.s{1}p".format(self.project_name, excitations))
                        self.osolution.ExportNetworkData(variation_array[0], ["{0}:{1}".format(s, sweep)], 3,
                                                         szExportPath, ["All"], True, 50, "S", -1, 0, 15, True, False,
                                                         False)
                    ...
                    for variation in variation_array:
                        ...
                        self.odesign.ExportProfile(s, variation, szExportPath)
                        self.logger.info("Export Profile: %s", szExportPath)
                        szExportPath = os.path.join(export_folder, "{0}_{1}.conv".format(self.project_name, varCount))
                        self.logger.info("Export Convergence: %s", szExportPath)
                        self.odesign.ExportConvergence(s, variation, szExportPath)
                        try:
                            szExportPath = os.path.join(export_folder,
                                                        "{0}_{1}.s{2}p".format(self.project_name, varCount,
                                                                               excitations))
                            self.logger.info("Export SnP: {}".format(szExportPath))
                            self.osolution.ExportNetworkData(variation, ["{0}:{1}".format(s, sweep)], 3, szExportPath,
