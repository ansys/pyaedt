...
def create_stackup_settings_ui(tab_frame, app_instance):
    ...
    app_instance.layer_count = ttk.Combobox(layer_frame, width=5, values=list(range(2, 21)), style="PyAEDT.TCombobox")
    app_instance.layer_count.grid(row=0, column=1, padx=5, pady=5, sticky='w')
    app_instance.layer_count.set(4)  # Default to 4-layer PCB
    ...
    app_instance.select_all_var = tk.BooleanVar()
    select_all_check = ttk.Checkbutton(table_buttons_frame, text="Select All", variable=app_instance.select_all_var, 
    ...
    app_instance.stackup_tree = ttk.Treeview(table_frame, columns=columns, show='headings', selectmode='extended', style="PyAEDT.Treeview")
    
    # Configure checkbox column for layer selection
    app_instance.stackup_tree.column('Select', width=80, stretch=True, anchor='center')
    app_instance.stackup_tree.heading('Select', text='Select')
    
    # Configure index column
    app_instance.stackup_tree.column('Index', width=50, stretch=True)
    app_instance.stackup_tree.heading('Index', text='#')
    ...
    def handle_selection_shortcut(event):
        if event.state & 4:  # Ctrl key
            if event.keysym == 'a':  # Ctrl+A
                app_instance.stackup_tree.selection_set(app_instance.stackup_tree.get_children())
                return 'break'
    
    app_instance.stackup_tree.bind('<Control-a>', handle_selection_shortcut)
    
    # Add Shift+Click support for range selection
    app_instance.last_selected = None
    
    def handle_shift_click(event):
        if event.state & 1:  # Shift key
            item = app_instance.stackup_tree.identify_row(event.y)
            if item and app_instance.last_selected:
                items = app_instance.stackup_tree.get_children()
                try:
                    start_idx = items.index(app_instance.last_selected)
                    end_idx = items.index(item)
                    if start_idx > end_idx:
                        start_idx, end_idx = end_idx, start_idx
                    for i in range(start_idx, end_idx + 1):
                        app_instance.stackup_tree.selection_add(items[i])
                except ValueError:
                    pass
                return 'break'
        app_instance.last_selected = app_instance.stackup_tree.identify_row(event.y)
    
    app_instance.stackup_tree.bind('<ButtonRelease-1>', handle_shift_click)
    
    # Initialize selection states for toggle buttons
    app_instance.metal_selected = False
    app_instance.dielectric_selected = False
    
    # Add layer type selection buttons with fixed width to prevent layout shifts
    app_instance.select_metal_button = ttk.Button(table_buttons_frame, text="Select Metal", width=15,
                                   command=lambda: toggle_select_by_type(app_instance, 'signal'), style="PyAEDT.TButton")
    app_instance.select_metal_button.grid(row=0, column=6, padx=5, sticky='w')
    
    app_instance.select_dielectric_button = ttk.Button(table_buttons_frame, text="Select Dielectric", width=15,
                                         command=lambda: toggle_select_by_type(app_instance, 'dielectric'), style="PyAEDT.TButton")
    app_instance.select_dielectric_button.grid(row=0, column=7, padx=5, sticky='w')
    ...
    for col in columns:
        if col != 'Index':  # Skip already configured columns
            ...
            if col == 'Name':
                ...
                width = 100
            app_instance.stackup_tree.column(col, width=width, stretch=True)
    
    # Initialize checkbox widgets dictionary
    app_instance.checkbox_widgets = {}

    # Add vertical scrollbar for table
    scrollbar = ttk.Scrollbar(table_frame, orient='vertical', command=app_instance.stackup_tree.yview, style="PyAEDT.Vertical.TScrollbar")
    app_instance.stackup_tree.configure(yscrollcommand=scrollbar.set)
    ...
    def handle_single_click(event):
        region = app_instance.stackup_tree.identify_region(event.x, event.y)
        if region == "cell":
            column = app_instance.stackup_tree.identify_column(event.x)
            if column == '#1':  # Select column (checkbox column)
                item = app_instance.stackup_tree.identify_row(event.y)
                if item:
                    # Toggle checkbox state
                    current_state = app_instance.checkbox_states.get(item, False)
                    new_state = not current_state
                    app_instance.checkbox_states[item] = new_state
                    
                    # Update checkbox symbol
                    values = list(app_instance.stackup_tree.item(item, 'values'))
                    values[0] = "☑" if new_state else "☐"
                    app_instance.stackup_tree.item(item, values=values)
                    
                    # Update selection
                    if new_state:
                        app_instance.stackup_tree.selection_add(item)
                    else:
                        app_instance.stackup_tree.selection_remove(item)
                    
                    # Update select all checkbox state
                    update_select_all_state(app_instance)
    
    # Bind events
    app_instance.stackup_tree.bind('<Button-1>', on_single_click)
    app_instance.stackup_tree.bind('<Double-1>', on_double_click)
    ...
    try:
        ...
        app_instance.diagram_image = ImageTk.PhotoImage(stackup_demo_image)
        stackup_image_label = ttk.Label(image_frame, image=app_instance.diagram_image, style="PyAEDT.TLabel")
...
def update_select_all_state(app_instance):
    ...
    Args:
        app_instance: The main application instance
    """
    if not hasattr(app_instance, 'checkbox_states'):
        return
    
    all_items = app_instance.stackup_tree.get_children()
    if not all_items:
        app_instance.select_all_var.set(False)
        return
    
    checked_count = sum(1 for item in all_items if app_instance.checkbox_states.get(item, False))
    
    if checked_count == len(all_items):
        app_instance.select_all_var.set(True)
    else:
        app_instance.select_all_var.set(False)
...
def toggle_all_selections(app_instance):
    ...
    """
    if not hasattr(app_instance, 'checkbox_states'):
        app_instance.checkbox_states = {}
    if not hasattr(app_instance, 'checkbox_widgets'):
        app_instance.checkbox_widgets = {}
    
    select_all = app_instance.select_all_var.get()
    
    # Reset other button states when Select All is activated
    if select_all:
        app_instance.metal_selected = False
        app_instance.dielectric_selected = False
        app_instance.select_metal_button.config(text="Select Metal")
        app_instance.select_dielectric_button.config(text="Select Dielectric")
    
    for item in app_instance.stackup_tree.get_children():
        # Update checkbox state
        app_instance.checkbox_states[item] = select_all
...
def toggle_select_by_type(app_instance, layer_type):
    ...
    if layer_type == 'signal':
        current_selected = app_instance.metal_selected
        app_instance.metal_selected = not current_selected
        target_state = app_instance.metal_selected
        button = app_instance.select_metal_button
        ...
        current_selected = app_instance.dielectric_selected
        app_instance.dielectric_selected = not current_selected
        target_state = app_instance.dielectric_selected
        button = app_instance.select_dielectric_button
    ...
    if target_state:
        ...
        for item in app_instance.stackup_tree.get_children():
            app_instance.checkbox_states[item] = False
            values = list(app_instance.stackup_tree.item(item)['values'])
            values[0] = "☐"
            app_instance.stackup_tree.item(item, values=values)
            app_instance.stackup_tree.selection_remove(item)
    ...
    for item in app_instance.stackup_tree.get_children():
        values = app_instance.stackup_tree.item(item)['values']
        # Check if this item matches the layer type (Select=0, Index=1, Name=2, Type=3)
        if len(values) > 3 and values[3] == layer_type:
            # Update checkbox state
            app_instance.checkbox_states[item] = target_state
            # Update display
            new_values = list(values)
            new_values[0] = "☑" if target_state else "☐"
            app_instance.stackup_tree.item(item, values=new_values)
            # Update selection
            if target_state:
                app_instance.stackup_tree.selection_add(item)
            else:
...
def select_by_type(app_instance, layer_type):
    ...
    app_instance.stackup_tree.selection_clear()
    ...
    for item in app_instance.stackup_tree.get_children():
        ...
        if len(values) > 3 and values[3] == layer_type:
            ...
            app_instance.stackup_tree.selection_add(item)
            # Update checkbox state
            app_instance.checkbox_states[item] = True
            # Update checkbox symbol
            updated_values = list(values)
            updated_values[0] = "☑"
            app_instance.stackup_tree.item(item, values=updated_values)
        else:
            # Uncheck this item
            app_instance.checkbox_states[item] = False
            # Update checkbox symbol
...
def batch_edit_layers(app_instance):
    ...
    Args:
        app_instance: The main application instance
    """
    selected_items = app_instance.stackup_tree.selection()
    ...
    def save_changes():
        for item in selected_items:
            idx = app_instance.stackup_tree.index(item)
...
def update_stackup_tree(app_instance):
    ...
    if hasattr(app_instance, 'metal_selected'):
        app_instance.metal_selected = False
        app_instance.select_metal_button.config(text="Select Metal")
    if hasattr(app_instance, 'dielectric_selected'):
        app_instance.dielectric_selected = False
        app_instance.select_dielectric_button.config(text="Select Dielectric")
    
    # Populate table with current stackup layer data
    for layer_index, stackup_layer in enumerate(app_instance.config_model.stackup, 1):
        tree_item_id = app_instance.stackup_tree.insert("", "end", values=(
        ...
        app_instance.checkbox_states[tree_item_id] = False
...
def edit_layer(app_instance):
    ...
    selected_layer_index = app_instance.stackup_tree.index(selection[0])
...
def move_layer_up(app_instance):
    ...
    current_layer_index = app_instance.stackup_tree.index(selection[0])
    if current_layer_index > 0:
        ...
        tree_items = app_instance.stackup_tree.get_children()
        app_instance.stackup_tree.selection_set(tree_items[current_layer_index-1])
