    ...
    for boundary_name, boundary_details in data.get("boundaries", {}).items():
        ...
        if boundary_type == "Block":
            ...
            if use_total_power:
                ...
                boundary_data.append([boundary_name, boundary_type, selected_objects,
                                      bc_name1, value1, bc_name2, value2])
                cols.append(7)
...
def extract_material_data(data):
    ...
    extracted_data = []
    read_only_cols = []
    headings = ["Material Name", "Material Type", "Thermal Conductivity", "Mass Density", "Specific Heat",
                "Thermal Expansion Coefficient",  "Diffusivity", "Viscosity"]
    ...
    for mat_name, mat_details in materials.items():
        ...
        extracted_data.append([mat_name, material_type, thermal_conductivity, mass_density, specific_heat,
            thermal_expansion_coefficient, diffusivity, viscosity
        ])
        read_only_cols.append(cols)
...
def compare_and_update_boundary_data(original_data, modified_object_data, object_mapping):
    ...
    for row in modified_object_data:
        (boundary_name, boundary_type, selected_objects,
                                      bc_name1, value1, bc_name2, value2) = row
        ...
        if isinstance(selected_objects, str):
            selected_object_ids = [int(i.strip()) if str(i.strip()).isdigit()  else object_mapping[i.strip()]
        ...
        if original_obj_ids:
            if set(original_obj_ids) != set(selected_object_ids):
                differences.append(
                    f"{boundary_name}: {bc_name1} selected objects changed  to '{selected_objects}'")
...
def compare_and_update_model_data(original_data, modified_object_data):
    ...
    for row in modified_object_data:
        ...
        if original_obj.get("Model", "") != modelling_bool:
            differences.append(f"{obj_name}: Modeling changed from '{original_obj.get('Model', '')}' to '{modelling_bool}'")
...
def compare_and_update_material_data(original_data, modified_object_data):
    ...
    for row in modified_object_data:
        (mat_name, material_type, thermal_conductivity,
         mass_density, specific_heat,thermal_expansion_coefficient, diffusivity, viscosity)= row
        ...
        if original_obj.get("thermal_expansion_coefficient", "") != thermal_expansion_coefficient:
            differences.append(f"{mat_name}: Thermal expansion coefficient changed "
        ...
        if original_obj.get("diffusivity", "") != diffusivity:
            differences.append(f"{mat_name}: diffusivity changed "
                               f"from '{original_obj.get('diffusivity', '')}' to '{diffusivity}'")
