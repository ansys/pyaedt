class Node_TwoRayPathLossCouplingNode(GenericEmitNode):
    ...
    @property
    def get_ground_reflection_coeff(self):
        """Ground Reflection Coeff.
        "The ground reflection coefficient."
        "Value should be between -100 and 100."
        """
        return oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Ground Reflection Coeff.')
    def set_ground_reflection_coeff(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Ground Reflection Coeff.=' + value])

    @property
    def get_pointspeak(self):
        """Points/Peak
        "Number of points used to model each peak in frequency vs loss curve."
        "Value should be between 3 and 100."
        """
        return oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Points/Peak')
    def set_pointspeak(self, value):
