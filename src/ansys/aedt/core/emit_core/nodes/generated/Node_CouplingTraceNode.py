class Node_CouplingTraceNode(GenericEmitNode):
    ...
    def get_highlight_regions(self):
        ...
        "Value should be 'true' or 'false'."
        """
        return oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Highlight Regions')
    def set_highlight_regions(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Highlight Regions=' + value])

    @property
    def get_show_region_labels(self):
        """Show Region Labels
        "If true, regions of the trace are labelled."
        "Value should be 'true' or 'false'."
        """
        return oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Show Region Labels')
    def set_show_region_labels(self, value):
