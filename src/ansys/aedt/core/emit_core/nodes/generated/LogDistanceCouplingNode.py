...
class LogDistanceCouplingNode(GenericEmitNode):
    ...
    def path_loss_exponent(self) -> float:
        ...
        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Path Loss Exponent')
        key_val_pair = [i for i in props if 'Path Loss Exponent=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    @path_loss_exponent.setter
    def path_loss_exponent(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Path Loss Exponent=' + value])
