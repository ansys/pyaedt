...
class ReadOnlyIndoorPropagationCouplingNode(GenericEmitNode):
    ...
    def building_type(self):
        ...
        "        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Building Type')
        key_val_pair = [i for i in props if 'Building Type=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    ...
    def number_of_floors(self) -> int:
        ...
        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Number of Floors')
        key_val_pair = [i for i in props if 'Number of Floors=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
