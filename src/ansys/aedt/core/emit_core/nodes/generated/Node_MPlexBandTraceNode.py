class Node_MPlexBandTraceNode(GenericEmitNode):
    ...
    @property
    def get_power_direction(self):
        """Power Direction
        """
        return oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Power Direction')
    def set_power_direction(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Power Direction=' + value])
    class PowerDirectionOption(Enum):
        (
            INCOMMONPORT|OUTCOMMONPORT = "Into Common Port|Out of Common Port"
        )
