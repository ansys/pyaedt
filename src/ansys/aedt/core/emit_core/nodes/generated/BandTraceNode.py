...
class BandTraceNode(GenericEmitNode):
    ...
    def tx_or_rx(self):
        ...
        "        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Tx or Rx')
        key_val_pair = [i for i in props if 'Tx or Rx=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    @tx_or_rx.setter
    def tx_or_rx(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Tx or Rx=' + value])
