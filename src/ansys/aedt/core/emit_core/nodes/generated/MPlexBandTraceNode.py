...
class MPlexBandTraceNode(GenericEmitNode):
    ...
    def power_direction(self):
        """Power Direction
        "        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Power Direction')
        key_val_pair = [i for i in props if 'Power Direction=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    @power_direction.setter
    def power_direction(self, value):
