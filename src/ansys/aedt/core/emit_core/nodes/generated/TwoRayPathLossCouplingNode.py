...
class TwoRayPathLossCouplingNode(GenericEmitNode):
    ...
    def ground_reflection_coeff(self) -> float:
        ...
        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Ground Reflection Coeff.')
        key_val_pair = [i for i in props if 'Ground Reflection Coeff.=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    @ground_reflection_coeff.setter
    def ground_reflection_coeff(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Ground Reflection Coeff.=' + value])
    ...
    def pointspeak(self) -> int:
        ...
        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Points/Peak')
        key_val_pair = [i for i in props if 'Points/Peak=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    @pointspeak.setter
    def pointspeak(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Points/Peak=' + value])
