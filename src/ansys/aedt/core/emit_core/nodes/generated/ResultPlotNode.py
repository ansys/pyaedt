...
class ResultPlotNode(GenericEmitNode):
    ...
    def show_emi_thresholds(self) -> bool:
        ...
        "Value should be 'true' or 'false'."
        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Show EMI Thresholds')
        key_val_pair = [i for i in props if 'Show EMI Thresholds=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
    @show_emi_thresholds.setter
    def show_emi_thresholds(self, value):
        oDesign.GetModule('EmitCom').SetProperties(self._result_id,self._node_id,['Show EMI Thresholds=' + value])
