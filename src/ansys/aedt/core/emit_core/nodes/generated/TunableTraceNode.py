...
class TunableTraceNode(GenericEmitNode):
    ...
    def frequency(self) -> float:
        ...
        "        """
        props = oDesign.GetModule('EmitCom').GetProperties(self._result_id,self._node_id,'Frequency')
        key_val_pair = [i for i in props if 'Frequency=' in i]
        if len(key_val_pair) != 1:
            return ''
        val = key_val_pair[1].split('=')[1]
        return val
