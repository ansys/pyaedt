...
class RFViaOpt:
    ...
    def __init__(self):
        ...
        self.edb = Edb(edbpath=aedb_path, edbversion="2021.2")

    def _create_padstack(self, name, holediam="0.3mm", antipaddiam="0.7mm", paddiam="0.5mm"):
        self.edb.core_padstack.create_padstack(padstackname=name,
                                               holediam=holediam,
    ...
    def _create_trace_path(self, net_name, points, onlayer, width):

        path = self.edb.core_primitives.Shape("polygon", points=points)
        self.edb.core_primitives.create_path(path, layer_name=onlayer, net_name=net_name, width=width,
                                             start_cap_style="Flat",
                                             end_cap_style="Flat")

    def _create_ground_planes(self, layers):
        plane = self.edb.core_primitives.Shape("rectangle", pointA=["-3mm", "-3mm"], pointB=["3mm", "3mm"])

        for i in layers:
            self.edb.core_primitives.create_polygon(plane, i, net_name="GND")

    def run(self, layout_count=12,
            diel_material_name="FR4_epoxy",
            diel_thickness="0.15mm",
            cond_thickness_outer="0.05mm",
            cond_thickness_inner="0.017mm",
        ...
        # Create variables
        self.edb.add_design_variable("$via_hole_size", "0.3mm")
        self.edb.add_design_variable("$antipaddiam", "0.7mm")
        self.edb.add_design_variable("$paddiam", "0.5mm")
        self.edb.add_design_variable("via_pitch", "1mm")
        self.edb.add_design_variable("trace_in_width", "0.2mm")
        self.edb.add_design_variable("trace_out_width", "0.1mm")

        # Create padstacks
        self._create_padstack(name="SVIA",
                              holediam="$via_hole_size",
                              antipaddiam="$antipaddiam",
                              paddiam="$paddiam")
        self._create_padstack(name="GVIA")

        # Place signal via
        self.edb.core_padstack.place_padstack([0, 0], "SVIA", net_name="RF",
        ...
        if gvia_num_side % 2:
            ...
            self.edb.core_padstack.place_padstack(["via_pitch", 0], "GVIA", net_name="GND")
            self.edb.core_padstack.place_padstack(["via_pitch*-1", 0], "GVIA", net_name="GND")
            for i in np.arange(1, gvia_num_side / 2):
                ...
                self.edb.core_padstack.place_padstack([xloc, yloc], "GVIA", net_name="GND")
                self.edb.core_padstack.place_padstack([xloc, yloc + "*-1"], "GVIA", net_name="GND")

                self.edb.core_padstack.place_padstack([xloc + "*-1", yloc], "GVIA", net_name="GND")
                self.edb.core_padstack.place_padstack([xloc + "*-1", yloc + "*-1"], "GVIA", net_name="GND")
        else:
        ...
        self._create_trace_path(net_name="RF", points=[[0, 0], [0, "-3mm"]], onlayer=trace_in_layer,
                                width="trace_in_width")
        self._create_trace_path(net_name="RF", points=[[0, 0], [0, "3mm"]], onlayer=trace_out_layer,
        ...
        self._create_ground_planes(layers=ground_layers)

    def save_edb_as(self, fpath):
        self.edb.save_edb_as(fpath)
        self.edb.close_edb()



app = RFViaOpt()
app.run(layout_count=12,
        diel_material_name="FR4_epoxy",
        diel_thickness="0.15mm",
        cond_thickness_outer="0.05mm",
        cond_thickness_inner="0.017mm",
...
app.save_edb_as(os.path.join(temp_fold, "via_opt.aedb"))
print("aedb Saved in {}".format(os.path.join(temp_fold, "via_opt.aedb")))
