...
M3D = Maxwell3d(projectname=Project_Name,designname=Design_Name,solution_type=Solver,specified_version=DesktopVersion)
...
M3D.modeler.duplicate_around_axis('Coil',cs_axis='Global',angle=90,nclones=4,create_new_objects=True,is_3d_comp=False)
...
test.SetEddyEffect (["NAME:Eddy Effect Setting",
    ["NAME:EddyEffectVector",["NAME:Data","Object Name:=","Plate","Eddy Effect:=",True,"Displacement Current:=",False],
...
dataset = ['Bz A1_B1 000 0',
            'Bz A1_B1 050 0','Bz A1_B1 050 90','Bz A1_B1 200 0','Bz A1_B1 200 90',
            'Bz A2_B2 050 0','Bz A2_B2 050 90','Bz A2_B2 200 0','Bz A2_B2 200 90']
header = ['Distance [mm]','Bz [Tesla]']

line_length = [0,18,36,54,72,90,108,126,144,162,180,198,216,234,252,270,288]
data = [[-6.667,-7.764,-8.707,-8.812,-5.870,8.713,50.40,88.47,100.9,104.0,104.8,104.9,104.6,103.1,97.32,75.19,29.04],
        [4.90,-17.88,-22.13,-20.19,-15.67,0.36,43.64,78.11,71.55,60.44,53.91,52.62,53.81,56.91,59.24,52.78,27.61],
        [-1.16,2.84,4.15,4.00,3.07,2.31,1.89,4.97,12.61,14.15,13.04,12.40,12.05,12.27,12.66,9.96,2.36],
        [-3.63,-18.46,-23.62,-21.59,-16.09,0.23,44.35,75.53,63.42,53.20,48.66,47.31,48.31,51.26,53.61,46.11,24.96],
        [-1.38,1.20,2.15,1.63,1.10,0.27,-2.28,-1.40,4.17,3.94,4.86,4.09,3.69,4.60,3.48,4.10,0.98],
        [-1.83,-8.50,-13.60,-15.21,-14.48,-5.62,28.77,60.34,61.84,56.64,53.40,52.36,53.93,56.82,59.48,52.08,26.56],
        [-1.63,-0.60,-0.43,0.11,1.26,3.40,6.53,10.25,11.83,11.83,11.01,10.58,10.80,10.54,10.62,9.03,1.79],
        [-0.86,-7.00,-11.58,-13.36,-13.77,-6.74,24.63,53.19,54.89,50.72,48.03,47.13,48.25,51.35,53.35,45.37,24.01],
...
for item in dataset_range:
    if item % 2 == 0:
        ...
        if dataset[item][9:12] == '000':
            Plot.CreateReport(plotname,"Fields","Rectangular Plot","Setup1 : LastAdaptive",
                              ["Context:=",'Line_' + dataset[item][3:8],"PointCount:=",1001],
                              ["Distance:=",["All"],"Freq:=",[str(dc_freq) + 'Hz'],"Phase:=",["0deg"],
                                  "Coil_Excitation:=",["All"]],["X Component:=","Distance","Y Component:=",[dataset[item][0:2]]])
        else:
            Plot.CreateReport(plotname,"Fields","Rectangular Plot","Setup1 : LastAdaptive",
                      ["Context:=",'Line_'+dataset[item][3:8],"PointCount:=",1001],["Distance:=",["All"],"Freq:=",[dataset[item][9:12] + 'Hz'],
                                  "Phase:=",["0deg","90deg"],"Coil_Excitation:=",["All"]],
