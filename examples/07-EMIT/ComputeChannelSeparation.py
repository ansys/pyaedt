...
# Perform required imports.
#
# sphinx_gallery_thumbnail_path = "Resources/emit_channel_separation.png"
import os
import sys
import subprocess
import pyaedt
...
def get_rx_bands(rx_radio):
    ...
    """
    bands = rev.get_band_names(rx_radio, TxRxMode.RX)
...
def minimum_tx_channel_separation(rx_band, tx_band, emi_threshold):
    ...
    tx_frequencies = rev.get_active_frequencies(
        tx_band[0], tx_band[1], TxRxMode.TX
    )
...
for rx_band, tx_band in tqdm(overlapping[1:]):
    tx_frequencies = rev.get_active_frequencies(tx_band[0], tx_band[1], TxRxMode.TX)
    rx_frequencies = rev.get_active_frequencies(rx_band[0], rx_band[1], TxRxMode.RX)
    print('Rx:')
