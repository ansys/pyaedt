...
inner_sheat_radius = filling_radius+0.75
armour_thickness = 3
armour_radius = inner_sheat_radius+armour_thickness
...
outer_params = {
	...
	"inner_sheat_radius": str(inner_sheat_radius) +'mm',
	"armour_radius": str(armour_radius) +'mm',
	"outersheat_radius": str(outersheat_radius)+'mm'
}
arm_params = {
...
pName = 'Q2D_ArmouredCableExample'
q2d_dname = '2D_Extractor_Cable'
sName = "MySetupAuto"
swName = "sweep1"
twinb_dname = 'CableSystem'
Q2D_DS = pyaedt.Q2d(projectname=pName,designname=q2d_dname,specified_version=desktopVersion)
...
fill_id = mod2D.create_circle(['0mm','0mm','0mm'],'3*c_strand_radius',name='c_strand_fill', matname='plastic_pp_carbon_fiber')
fill_id.color=(255,255,0)
xlpe_id = mod2D.create_circle(['0mm','0mm','0mm'],'3*c_strand_radius+'+str(core_xlpe_ins_thickness)+'mm',name='c_strand_xlpe',
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
arm_fill_id = mod2D.create_circle(['0mm','0mm','0mm'],'armour_radius',name='ArmourFilling', matname='plastic_pp_carbon_fiber')
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
arm_strand_1_id = mod2D.create_circle(['0mm','armour_centre_pos','0mm'],'1.1mm',name='arm_strand_1', matname='steel_stainless')
...
port_info_listA = []
port_info_listB = []
for i_var in range(len(sl_name_list)):
	port_info_listA.append("OnePortInfo:=")
	port_info_listB.append("OnePortInfo:=")
	my_listA = [str(sl_name_list[i_var])+'_'+str(sl_name_list[i_var])+'_A',-1,str(sl_name_list[i_var])+':'+str(sl_name_list[i_var])+'_A']
	my_listB = [str(sl_name_list[i_var]) + '_' + str(sl_name_list[i_var]) + '_B', -1,str(sl_name_list[i_var]) + ':' + str(sl_name_list[i_var]) + '_B']
...
for i_var in range(len(des_var_list)):
	 my_list = [des_var_list_sorted[i_var],str(des_var_list[des_var_list_sorted[0]].numeric_value)+des_var_list[des_var_list_sorted[0]].units]
...
dyn_component.AddDynamicNPortData(
	[
		...
		"name:="		, name_comp,
		"filename:="		, "$PROJECTDIR\\"+pName+'.aedt',
		"numberofports:="	, num_ports,
		"Is3D:="		, False,
		"IsWBLink:="		, False,
		"WBSystemId:="		, "",
		"CouplingDesignName:="	, q2d_dname,
