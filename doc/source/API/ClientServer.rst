...
4. CPython to Ironpython can be used with some data type limitations and potential issues. As an example, when getting
   lists from a command you can access every item (eg. mylist[i]) and len (eg. mylist.__len__()) but not len(mylist).
...
Linux Cpython Server is also supported but allows only to run Pyaedt in a script (no interactive Session).
